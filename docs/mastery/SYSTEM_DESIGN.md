...
- **FR-1: User Management**: Registration and login using salted `bcrypt` password hashing and signed JWT bearer tokens.
...
- **FR-6: Job Description Keyword Matching**: Calculate semantic keyword overlap, missing skills, and recommendations.
- **FR-7: Career Coaching Chat**: Context-grounded conversational career assistant providing targeted resume guidance.
- **FR-8: Vector PDF Export**: Generate print-ready, pagination-aware vector PDF documents via browser print isolation.

### 1.2 Non-Functional Requirements (NFR)
- **NFR-1: Upstream Isolation (Bulkhead)**: Active concurrent calls to Google Gemini must not exceed $\mathbf{4}$ per process, preventing event loop and API quota saturation.
...
- **NFR-5: Data Durability**: Persistent documents must survive container recreation via host-mounted volumes (`./backend:/app`).
...
| **BFF Layer** | **Thin Express Gateway** | Direct FastAPI Serving / NGINX | Bundles React SPA static assets cleanly; provides local dev proxying without extra NGINX container. | Adds a network hop (`:3000` $\to$ `:8000`) in Docker bridge network. | High-throughput deployment with dedicated NGINX edge caching. |
...
| **Authentication** | **Stateless JWT Bearer** | Server-Side Sessions | Stateless horizontal scaling; zero database session lookups on every HTTP request. | Token revocation requires token blacklists or short expiration windows ($60\text{min}$). | Systems requiring immediate instant session revocation. |
...
| **Redis Container Crash / Partition** | `redis.exceptions.ConnectionError` | `is_healthy()` flag flips to `False`. `rate_limiter.py` and `idempotency.py` switch to in-memory `dict` storage. | Zero 500 errors; rate limiting & idempotency continue in process memory. |
...
| **Simultaneous AI Spike** | `BulkheadPool` semaphore bound | First 4 execute; next 12 queue; remaining rejected at capacity boundary. | In-capacity requests process; excess receive `503 AI_CAPACITY_EXCEEDED` with `Retry-After: 5`. |

---

## 4. Bottlenecks & Capacity Mathematics (Example Capacity-Planning Scenario)
...
$$\text{Max Theoretical Write Throughput} = \frac{1}{t_{\text{write}}} \approx \frac{1}{0.022\text{s}} \approx \mathbf{45.4\text{ writes/second}}$$

*Scaling Trigger*: When sustained write traffic exceeds **$45\text{ writes/sec}$**, SQLite should be migrated to PostgreSQL 16.

### 4.2 Bulkhead Concurrency Mathematics
$$\text{Max In-Flight AI Work per Process} = \text{MAX\_CONCURRENT} + \text{MAX\_QUEUE\_DEPTH} = 4 + 12 = \mathbf{16\text{ requests}}$$
...
## 5. System Evolution Roadmap (Phases 1 to 5) [FUTURE SCALING]

```
Phase 1: Current Implemented Baseline
...
Phase 2: Relational Database Scaling [FUTURE] (Trigger: >45 writes/sec)
  ├── Replace SQLite with Managed PostgreSQL 16
  ├── Use asyncpg connection pooling + PgBouncer
  └── Execute schema migrations via Alembic

Phase 3: Horizontal API Scaling [FUTURE] (Trigger: >5,000 active users)
  ├── Deploy multiple stateless FastAPI container replicas
  ├── Front instances with NGINX / Cloud Load Balancer
  └── Redis Cluster for centralized rate limiting & idempotency across replicas

Phase 4: Asynchronous Task Queue Extraction [FUTURE] (Trigger: Background PDF Compilation)
...
Phase 5: Microservices Decomposition [FUTURE] (Trigger: Organizational Team Boundaries)
...
*Evidence-backed backend engineering gaps identified during the codebase audit:*
...
| **GAP-002** | Redis Fallback Multi-Instance | `rate_limiter.py` in-memory fallback uses process-local `dict`. | If Redis fails in a multi-replica deployment, effective rate limit becomes $N \times$ configured limit. | Document AP consistency trade-off in operational runbooks. | `[IMPORTANT]` |
| **GAP-003** | Asynchronous PDF Rendering | PDF export relies on client-side browser print engine. | Large multi-page resumes with heavy styling cannot be generated headlessly via backend background workers. | Introduce headless Chromium / WeasyPrint worker queue with 202 Accepted polling. | `[ADVANCED]` |
...
> "IntelliResume is an AI resume backend platform built with FastAPI, Redis 7, SQLite WAL, and React 19. It acts as an authoritative system-design reference architecture, implementing atomic Redis Lua rate limiting, distributed idempotency, request coalescing via `asyncio.Future`, bulkhead semaphore concurrency isolation, and atomic SQL Compare-And-Swap optimistic concurrency control to guarantee zero lost updates and sub-10ms fast-fail recovery."
...
> 
> To protect against AI quota exhaustion and latency spikes, every AI request traverses a 7-stage resilience pipeline: Redis Lua fixed-window rate limiting, atomic `SET NX EX` idempotency locking, in-flight request coalescing to eliminate thundering herds, a process bulkhead pool limiting active Gemini calls to 4, a 3-state circuit breaker failing fast in $<6\text{ms}$ during outages, and bounded retries with full jitter. 
> 
> Persistent resumes use SQLite in WAL mode with atomic Compare-And-Swap version checks (`WHERE version = :client_ver`), preventing multi-tab write conflicts with HTTP 409 responses. If Redis crashes, the system gracefully degrades to in-memory counters without throwing 500 errors."

### 7.3 Top 5 System Design Defense Questions
1. **"Why use SQLite instead of PostgreSQL?"**
   - *Defense*: SQLite in WAL mode eliminates external database server management while easily supporting single-node capacity of ~45 writes/sec. We established clear migration triggers to PostgreSQL 16 when sustained write concurrency demands it.
